...
begin
   ...
   RemoteName := ListBoxRemFiles.Items.Strings[ListBoxRemFiles.ItemIndex];
   if RemoteName[1] = '<' then
   begin
      Delete(RemoteName, 1, 7); { // Remove first seven characters, '<DIR>  ' }
      if RemoteName[1] = '.' then Exit;
   end;
   ...
   begin
      Caption := 'Rename File';
      LabelWhat.Caption := 'New filename:';
      EditLine.Text := RemoteName;

      if ShowModal = mrOk then
      begin
         if EditLine.Text = RemoteName then
            Exit;

         SFTP1.RemoteFile := RemoteName;
         try
...
begin
   ...
   begin
      ...
      begin
         try
            if IsDir then
               SFTP1.RemoveDirectory(RemoteName)
            else
               SFTP1.DeleteFile(RemoteName);
         except on E: Exception do
            UpdateNotes(E.Message);
         end;
         UpdateRemote;
